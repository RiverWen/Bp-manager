 ...
 * 改动说明：1339为新增一的行，目的是为了X轴刻度分2行。543、1332改为强制执行，1467、1468、1476、1501做过改动。
 * 数据入口的categories由原来的一维数组改为二维。2019-11-17  By River
 */
...
var config = {
    ...
    colors: ['#fca104', '#2e9c09', '#434348', '#90ed7d', '#f15c80', '#8085e9'],
...
function calCategoriesData(categories, opts, config) {
    ...
    if (maxTextLength + 2 * config.xAxisTextPadding > eachSpacing ||true) {//这里||true是我加的，强制
...
function drawXAxis(categories, opts, config, context) {
    ...
    if (config._xAxisTextAngle_ != 0 || true) {//原文件这里是===0，我改了一下为了强制执行，1339是另加的。
        context.beginPath();
        context.setFontSize(config.fontSize);
        context.setFillStyle(opts.xAxis.fontColor || '#666666');
        categories.forEach(function (item, index) {
            var offset = eachSpacing / 2 - measureText(item) / 2;
            context.fillText(item[0], xAxisPoints[index] + offset+15, startY + config.fontSize + 5);
            context.fillText(item[1], xAxisPoints[index] + offset+15, startY + config.fontSize + 16);
        });
        context.closePath();
        context.stroke();
    } else {
        ...
        itemList.forEach(function (item) {
            switch (opts.type) {
                case 'line':
                    ...
                    context.setLineWidth(1);
                    context.setStrokeStyle(item.color);
                    context.moveTo(startX - 2, startY -15);//这里-15原来为+5，我改的为了上移图例
                    context.lineTo(startX + 17, startY -15);//这里-15原来为+5...
                    context.stroke();
                    context.closePath();
                    context.beginPath();
                    context.setLineWidth(1);
                    context.setStrokeStyle('#ffffff');
                    context.setFillStyle(item.color);
                    context.moveTo(startX + 7.5, startY + 15);
                    context.arc(startX + 7.5, startY -15, 4, 0, 2 * Math.PI);//这里-15原来为+5...
                    context.fill();
                    context.stroke();
                    context.closePath();
                    break;
            ...
            context.fillText(item.name, startX, startY -11);//这里的-11原来为+9...
            context.closePath();
            context.stroke();
